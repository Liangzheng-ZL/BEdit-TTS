...
class LoadInputsAndTargets(object):
        ...
        for uttid, info in batch:
            ...
            if self.load_output:
                ...
                for idx, inp in enumerate(info['output']):
                    ...
                    elif inp['name'] == 'position':
                        x = np.array(inp['position'], dtype=np.int64)
                    elif inp['name'] == 'phn_position':
                        x = np.array(inp['phn_position'], dtype=np.int64)
                    elif 'tokenid' in inp:
    ...
    def _create_batch_tts(self, x_feats_dict, y_feats_dict, uttid_list, eos):
        ...
        positions = y_feats_dict['position']
        positions = [positions[i] for i in nonzero_sorted_idx]
        phn_position_name = 'phn_position'
        phn_positions = y_feats_dict['phn_position']
