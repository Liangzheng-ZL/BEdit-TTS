...
class LoadInputsAndTargets(object):
            ...
            for uttid, info in batch:
                ...
                if self.load_output:
                    ...
                    for idx, inp in enumerate(info['output']):
                        ...
                        elif inp['name'] == 'frame_positions':
                            x = np.array(inp['frame_positions'], dtype=np.int64)
                            mask_word_num = random.randint(1, len(x))
                            mask_word_index = random.randint(0, len(x) - mask_word_num)
                            mask_f_p1 = x[mask_word_index][0]
                            mask_f_p2 = x[mask_word_index + mask_word_num - 1][1]
                            x = np.array([mask_f_p1, mask_f_p2], dtype=np.int64)
                        elif inp['name'] == 'phone_positions':
                            x = np.array(inp['phone_positions'], dtype=np.int64)
                            mask_p_p1 = x[mask_word_index][0]
                            mask_p_p2 = x[mask_word_index + mask_word_num - 1][1]
                            x = np.array([mask_p_p1, mask_p_p2], dtype=np.int64)
                        elif 'tokenid' in inp:
    ...
    def _create_batch_tts(self, x_feats_dict, y_feats_dict, uttid_list, eos):
        ...
        spkids = None
        spkid_name = 'spkid'
        spkids = y_feats_dict['speaker_id']
        spkids = [spkids[i] for i in nonzero_sorted_idx]

        frame_position_name = 'frame_positions'
        frame_positions = y_feats_dict['frame_positions']
        frame_positions = [frame_positions[i] for i in nonzero_sorted_idx]

        phn_position_name = 'phone_positions'
        phn_positions = y_feats_dict['phone_positions']
        phn_positions = [phn_positions[i] for i in nonzero_sorted_idx]
        ...
        return_batch = OrderedDict([(x_name, xs),
                                    (spkid_name, spkids),
                                    (frame_position_name, frame_positions),
                                    (phn_position_name, phn_positions),
                                    (y_name, ys),
                                    (durs_name, durs),
